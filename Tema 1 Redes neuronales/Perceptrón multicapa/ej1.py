...
#tengo q tener en cuenta q en el multicapa las salidas de la capa de entrada son las entradas
#de la capa oculta y q las salidas d la capa oculta son las entradas de la capa de salida
ubi = "C:/Users/conra/OneDrive/Desktop/Facu Conrado/CUARTO AÑO/Inteligencia Computacional/Tema 1 Redes neuronales/Perceptrón multicapa/XOR_trn.csv"
ubtest = "C:/Users/conra/OneDrive/Desktop/Facu Conrado/CUARTO AÑO/Inteligencia Computacional/Tema 1 Redes neuronales/Perceptrón multicapa/XOR_tst.csv"

#defino la sigmoide simetrica q es la q usamos en la catedra de -1 a +1
# nos da una salida continua con valor -1 o +1 q dsp con la funcion signo le doy un valor exacto digamosle
# de -1 o +1
def sigmoide(v, b=1):
...
# lo q hago es convertir a la salida del perceptron en +1 o -1 dependiendo del caso, es lo mismo q haciamos
# en el for del perceptron simple
def signo(v):
...
def leer_datos(ruta, n_salidas):

    df = pd.read_csv(ruta)
    datos = df.to_numpy()
    # col totales
    n_columnas = datos.shape[1]
    # las ultimas col corresponden a las salidas
    n_entradas = n_columnas - n_salidas
    X = datos[:, :n_entradas]
    D = datos[:, n_entradas:]
    return X, D

#para las distintas capas debo crear la matriz de pesos correspondientes
...
def ej1(capas,ruta_train,ruta_test,vel_aprendizaje,max_epocas,err_umbral,activacion):
    # CARGAR DATOS
    X_train, D_train = leer_datos(ruta_train,capas[-1])
    X_test, D_test = leer_datos(ruta_test,capas[-1])

    # INICIALIZAR PESOS
    w = inicializar_pesos(capas)
    ...
    for epoca in range(max_epocas):
        error_epoca = 0
        aciertos_epoca = 0
        # Recorremos todos los patrones de entrenamiento
        for n in range(n_patrones):
            # PROPAGACIÓN HACIA ADELANTE
            # La primera salida es simplemente la entrada
            y = [X_train[n]]
            # Recorremos todas las capas
            for p in range(len(capas) - 1):
                # Agregamos bias
                x = np.concatenate(([-1], y[p]))
                # Calculamos la salida de la capa
                y.append(propagacion_adelante(x,w[p],activacion))

            # CALCULO DEL ERROR
            # error = deseado - salida obtenida
            e = D_train[n] - y[-1]
            # error cuadratico del patron
            error_patron = 0.5 * np.sum(e**2)
            # acumulamos el error de la epoca
            error_epoca += error_patron

            # RETROPROPAGACION
            deltas = Retropropagacion(e,y,w)

            # ACTUALIZACION DE PESOS
            for p in range(len(w)):
                # Entrada a la capa incluyendo bias
                entrada_con_bias = np.concatenate(([-1], y[p]))
                # Actualizamos los pesos
                w[p] = ActualizarPesos(w[p],deltas[p],entrada_con_bias,vel_aprendizaje)
            # ACIERTOS DURANTE EL ENTRENAMIENTO
            if np.array_equal(signo(y[-1]),D_train[n]):
                aciertos_epoca += 1

        # RESULTADOS DE LA EPOCA
        tasa_aciertos = (aciertos_epoca / n_patrones)
        print(f"Época {epoca + 1}: "f"Error = {error_epoca:.6f}, "f"Aciertos = {tasa_aciertos * 100:.2f}%")

        # CRITERIO DE PARADA
        if error_epoca < err_umbral:
            print(f"Entrenamiento detenido "f"en la época {epoca + 1}")
            break

    # TEST
    aciertos, n_patrones_test = Probar(X_test,D_test,w,activacion)
    print(f"\nTest: {aciertos}/{n_patrones_test} "f"aciertos "f"({100 * aciertos / n_patrones_test:.2f}%)")

    # MOSTRAR PESOS FINALES
    for i, wp in enumerate(w):
        print(f"\nPesos de la capa {i + 1}:")
        print(wp)
    return w

# EJECUCION DEL EJERCICIO 1
capas = [2, 3, 3, 1]
w_finales = ej1(capas,ubi,ubtest,
    0.1,       # velocidad de aprendizaje
    500,       # maximo de epocas
    0.05,      # error minimo
    sigmoide   # funcion de activacion
)
